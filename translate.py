...
def parse_reqest(request_data):
    ...
    if "GET" == data_list[0]:
        content = data_list[1]
        if content.split("?")[0] == "/translate":
            type = content.split("&")[0].split("=")[1]
            input = content.split("&")[1][6:]
            print("ok")
            a = bytes(input,encoding='gbk')
            string = str(a, 'gbk')
            result =  net_translate(unquote(input),type)
            return unquote(str(input)) + ":" + result
        else:
            return None
    else:
        return "not support port request！"
def handle_client(client_socket):
    ...
    #解析收到的数据
    result = parse_reqest(request_data)
    # 构造响应数据
